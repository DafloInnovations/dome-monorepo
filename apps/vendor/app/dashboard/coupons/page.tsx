...
function CouponModal({
  ...
  const [form, setForm] = useState(initial ? {
    ...
    usageLimitPerUser: String(initial.usageLimitPerUser ?? 1),
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      const res = initial
        ? await apiFetch<{ data: Coupon }>(`/vendor/coupons/${initial.id}`, { method: "PUT", body: JSON.stringify(body) })
        : await apiFetch<{ data: Coupon }>("/vendor/coupons", { method: "POST", body: JSON.stringify(body) });
      onSave(res.data);
    } catch (err) {
        ...
        <form onSubmit={handleSave} className="p-6 space-y-4">
          <div className="flex gap-2">
            <div className="flex-1">
              ...
              <input className={inputCls} placeholder="VND-XXXX" value={form.code}
          ...
          <div>
            ...
            <input className={inputCls} placeholder="e.g. 10% off for members"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-muted mb-1">Facility ID (blank = all facilities)</label>
            ...
            <div>
              ...
              <select className={inputCls} value={form.sport} onChange={(e) => set("sport", e.target.value)}>
                ...
                {["BADMINTON","PICKLEBALL","TENNIS","BASKETBALL","SOCCER","VOLLEYBALL","HOCKEY","SQUASH"].map((s) => (
          ...
          <div className="p-3 bg-black/40 rounded-dome text-xs text-muted border border-border">
            ℹ️ This coupon will only be valid for your facilities. Players enter the code at checkout.
          </div>
...
export default function VendorCouponsPage() {
  ...
  const load = useCallback(() => {
    setIsLoading(true);
    apiFetch<{ data: Coupon[]; total: number }>("/vendor/coupons")
  ...
  async function handleDeactivate(id: string) {
    ...
    await apiFetch(`/vendor/coupons/${id}`, { method: "DELETE" });
    load();
  }
      ...
      <main className="flex-1 p-6 space-y-6 overflow-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: "Facility-specific", value: coupons.filter((c) => c.facilityId).length },
          ].map((s) => (
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted">
            Coupons give players a discount when booking your facilities.
          </p>
          <button
        ...
        {error && (
          <div className="bg-red-900/30 border border-red-700 rounded-dome px-4 py-3 text-red-400 text-sm">{error}</div>
        )}
        ...
        <div className="bg-surface border border-border rounded-dome overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={7} className="px-4 py-12 text-center text-muted">Loading…</td></tr>
              ) : coupons.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center">
                    <p className="text-2xl mb-2">🎟️</p>
                    <p className="text-white font-semibold">No coupons yet</p>
                    <p className="text-muted text-xs mt-1">Create your first discount coupon to attract more bookings</p>
                  </td>
                </tr>
              ) : (
                coupons.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className="border-b border-border last:border-0 hover:bg-white/[0.02]">
                      <td className="px-4 py-3">
                        ...
                        {c.description && <p className="text-xs text-muted mt-0.5 truncate max-w-[150px]">{c.description}</p>}
                      </td>
                      <td className="px-4 py-3 text-white">{couponValueLabel(c)}</td>
                      <td className="px-4 py-3 text-xs text-muted">
                        {c.facility ? c.facility.name : "All facilities"}
                        {c.sport && <span className="block text-[10px]">{c.sport}</span>}
                      </td>
                      <td className="px-4 py-3 text-white">{c.usedCount}/{c.usageLimit ?? "∞"}</td>
